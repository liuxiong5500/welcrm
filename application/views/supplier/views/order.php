<div class="panel_s">
    <div class="panel_body">
        <?php if ($order->status == 2) { ?>
            <a href="<?php echo site_url('suppliers/order_status/' . $order->id . '/4') ?>"
               class="btn btn-primary"><?php echo _l('supplier_purchase_order_accept'); ?></a>
            <a href="<?php echo site_url('suppliers/order_status/' . $order->id . '/3') ?>"
               class="btn btn-primary"><?php echo _l('purchase_order_status_no_ready'); ?></a>
        <?php }
        if ($order->status == 3 || $order->status == 4) { ?>
            <a href="<?php echo site_url('suppliers/order_status/' . $order->id . '/5') ?>"
               class="btn btn-primary"><?php echo _l('purchase_order_status_production'); ?></a>
        <?php } elseif ($order->status == 5) { ?>
            <a href="<?php echo site_url('suppliers/order_status/' . $order->id . '/6') ?>"
               class="btn btn-primary"><?php echo _l('purchase_order_status_finished'); ?></a>
        <?php } elseif ($order->status == 6) { ?>
            <a href="<?php echo site_url('suppliers/order_status/' . $order->id . '/7') ?>"
               class="btn btn-primary"><?php echo _l('purchase_order_status_departured'); ?></a>
        <?php } ?>
    </div>
</div>
<div class="panel_s">
    ...
    <div class="panel-body">
        <table class="table dt-table" data-order-col="2" data-order-type="desc">
            ...
            <?php foreach ($order->items as $key => $item) { ?>
                ...
                <td>
                    <a href="<?php echo site_url('suppliers/item_logs/' . $item['id']); ?>"><?php echo _l('view'); ?></a>
                </td>
            <?php } ?>
            </tbody>
        </table>
