...
class Suppliers_authentication_model extends CRM_Model
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('supplier_autologin');
        $this->autologin();
    }
    ...
    public function login($email, $password, $remember)
    {
        if ((!empty($email)) and (!empty($password))) {
            ...
            do_action('before_supplier_login', [
                'email' => $email,
                'userid' => $user->supplier_id,
                'supplier_contact_user_id' => $user->$_id,
            ]);

            $user_data = [
                'supplier_user_id' => $user->supplier_id,
                'supplier_contact_user_id' => $user->$_id,
            ...
            if (!$twoFactorAuth) {
                if ($remember) {
                    $this->create_autologin($user->$_id);
                }
    ...
    public function logout()
    {
        ...
        if (is_supplier_logged_in()) {
            do_action('before_supplier_logout', get_supplier_id());
    ...
    private function create_autologin($user_id)
    {
        ...
        $this->supplier_autologin->delete($user_id, $key);
        if ($this->supplier_autologin->set($user_id, md5($key))) {
            set_cookie([
                'name' => 'autologin',
                'value' => serialize([
                    'user_id' => $user_id,
    ...
    private function delete_autologin()
    {
        ...
        if ($cookie = get_cookie('autologin', true)) {
            ...
            $this->supplier_autologin->delete($data['user_id'], md5($data['key']));
        ...
        if (!is_logged_in()) {
            ...
            if ($cookie = get_cookie('autologin', true)) {
                ...
                if (isset($data['key']) and isset($data['user_id'])) {
                    if (!is_null($user = $this->supplier_autologin->get($data['user_id'], md5($data['key'])))) {
                        // Login user

                        // Get the customer id
                        $this->db->select('id');
                        $this->db->where('id', $user->id);
                        $contact = $this->db->get('tblsuppliercontacts')->row();

                        $user_data = [
                            'supplier_user_id' => $contact->supplier_id,
                            'supplier_contact_user_id' => $user->id,
                            'supplier_logged_in' => true,
                        ];
    ...
    public function set_password_email($email)
    {
        ...
        if ($user) {
            ...
            if ($this->db->affected_rows() > 0) {
                ...
                $merge_fields = [];
                $merge_fields = array_merge($merge_fields, get_supplier_contact_merge_fields($user->id, $user->$_id));

                $merge_fields = array_merge($merge_fields, get_password_merge_field($data, false, 'set'));
    ...
    public function forgot_password($email)
    {
        ...
        if ($user) {
            ...
            if ($this->db->affected_rows() > 0) {
                ...
                $template = 'supplier-forgot-password';

                $merge_fields = array_merge($merge_fields, get_password_merge_field($data, false, 'forgot'));
     ...
     */
    public function set_password($userid, $new_pass_key, $password)
    {
        if (!$this->can_set_password($userid, $new_pass_key)) {
            return [
                'expired' => true,
            ];
        }
     ...
     */
    public function reset_password($userid, $new_pass_key, $password)
    {
        if (!$this->can_reset_password($userid, $new_pass_key)) {
            return [
                'expired' => true,
            ];
        }
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $merge_fields = array_merge($merge_fields, get_client_contact_merge_fields($user->id, $user->$_id));
     ...
     */
    public function can_reset_password($userid, $new_pass_key)
    {
        $table = 'tblsuppliercontacts';
        $_id = 'id';
     ...
     * Check if the key is not expired or not exists in database
     */
    public function can_set_password($userid, $new_pass_key)
    {
        $table = 'tblsuppliercontacts';
        $_id = 'id';
        $this->db->where($_id, $userid);
