...
//        }
    ...
    public function update($data, $id)
    {
        ...
        $staff_levels = $this->levels_model->get_staff_levels($id);
        if (sizeof($staff_levels) > 0) {
            if (!isset($data['levels'])) {
                $this->db->where('staffid', $id);
                $this->db->delete('tblstafflevels');
            } else {
                foreach ($staff_levels as $staff_level) {
                    if (isset($levels)) {
                        if (!in_array($staff_level['levelid'], $levels)) {
                            $this->db->where('staffid', $id);
                            $this->db->where('levelid', $staff_level['levelid']);
                            $this->db->delete('tblstafflevels');
                            if ($this->db->affected_rows() > 0) {
                                $affectedRows++;
                            }
            ...
            if (isset($levels)) {
                foreach ($levels as $level) {
                    ...
                    $_exists = $this->db->get('tblstafflevels')->row();
                    if (!$_exists) {
                        $this->db->insert('tblstafflevels', [
                            'staffid' => $id,
                            'levelid' => $level,
