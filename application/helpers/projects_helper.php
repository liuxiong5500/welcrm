...
function get_purchase_order_status_by_id($id)
{
    $CI = &get_instance();
    ...
    $statuses = $CI->purchase_orders_model->get_statuses();
...
{
    $CI = &get_instance();
    if (!class_exists('payment_terms_model')) {
        $CI->load->model('payment_terms_model');
    }
    $payment_term = $CI->payment_terms_model->get($id);
    return $payment_term->name;
}

function get_shipment_term_name_by_id($id)
{
    $CI = &get_instance();
    if (!class_exists('shipment_terms_model')) {
        $CI->load->model('shipment_terms_model');
    }
    $shipment_term = $CI->shipment_terms_model->get($id);
...
{
    $CI = &get_instance();
    if (!class_exists('production_logs_model')) {
        $CI->load->model('production_logs_model');
    }
    $count = $CI->production_logs_model->get_finished_count($id);
    return $count->finished;
}

function get_purchase_order_item_shipped($id)
{
    $CI = &get_instance();
    if (!class_exists('production_logs_model')) {
        $CI->load->model('production_logs_model');
    }
    $count = $CI->production_logs_model->get_shipped_count($id);
...
{
    $CI = &get_instance();
    $warehouse = $CI->object_cache->get('warehouse-name-data-' . $id);

    if (!$warehouse) {
        $CI->db->select('name');
        $CI->db->where('id', $id);
        $warehouse = $CI->db->get('tblcustomerwarehouses')->row();
        $CI->object_cache->add('warehouse-name-data-' . $id, $warehouse);
    }
...
function customer_has_warehouses($customer_id)
{
    $totalCustomerWarehouses = total_rows('tblcustomerwarehouses', 'clientid=' . $customer_id);

    return ($totalCustomerWarehouses > 0 ? true : false);
}
